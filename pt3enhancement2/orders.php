...
<head>
  ...
  <title>Happiness Sweets Ordering System : Orders</title>
  <!-- Bootstrap -->
   ...
   <div class="row">
    ...
    <form id="createform" action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
          <input name="orderdate" type="date" class="form-control" id="orderdate" placeholder="" value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
        </div>
        </div>
    ...
    <div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2" id="my">
      <div class="page-header">
        <h2>Orders List</h2>
      </div>
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a176172, tbl_staffs_a176172_pt3, tbl_customers_a176172_pt3 WHERE ";
        $sql = $sql."tbl_orders_a176172.fld_staff_num = tbl_staffs_a176172_pt3.fld_staff_num and ";
        $sql = $sql."tbl_orders_a176172.fld_customer_num = tbl_customers_a176172_pt3.fld_customer_num";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a176172");
            $stmt->execute();
            $result = $stmt->fetchAll();
